/* sim-core.js: crossover and speaker-distortion maths for the viewer's Compare and Simulate views.
 *
 * No page code here, so it can be tested on its own (node tests/sim-core.test.js).
 * In the browser it becomes window.SimCore; in node it is module.exports.
 *
 ...
 *  - A driver's harmonic ratio at fundamental f comes from its measured curve (dB re fundamental,
 *    measured at level L0 at 1 m) and is moved to the level the driver actually plays at:
 *        HD_k(f, L) = HD_k(f, L0) + s_k * (L - L0)       (s_k in dB per dB)
...
(function (root, factory) {
  ...
  function wayResponses(f, points, aligned) {
  ...
  function logGrid(fLo, fHi, perOctave) {
  ...
  function slopeFromSweep(points, lo, hi) {
   ...
   *   ways: [{ name, L0, hd: {H2: [{x,y}], ...}, slope: (order, f) => dB per dB, count: identical drivers (1) }],
   *   ignoreBelowDb: -40
   * }
    ...
    freqs.forEach((f, fi) => {
      ...
      for (const k of orders) {
        ...
        ways.forEach((w, i) => {
          ...
          const curve = w.hd[k];
          const hd0 = curve ? interpLog(curve, f) : null;
          if (hd0 == null) { missing = true; contrib[i][k].push(null); gapsRaw.push({ way: i, order: k, f, fi }); return; }
          const ld = target + magDb;                          // what the way plays at f
          const each = ld - 20 * Math.log10(w.count || 1);   // identical drivers share it equally
          const hd = hd0 + w.slope(k, f) * (each - w.L0);
  ...
  const dbToPct = v => (v == null ? null : 100 * Math.pow(10, v / 20));

  return { TYPES, LAWS, wayResponses, logGrid, interpLog, slopeFromSweep, slopeCurve, simulate, dbToPct, _c: { C, abs, add } };
});
